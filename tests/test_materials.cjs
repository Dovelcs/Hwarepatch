...
const html=fs.readFileSync(process.env.PCB_TEST_HTML||'placement.html','utf8'),scripts=[...html.matchAll(/<script[^>]*>([\s\S]*?)<\/script>/g)].map(x=>x[1]);
...
toggleSelectionMark();if(![...selected].filter(ref=>get(ref).layer==='Top').every(ref=>done.has(ref))||[...selected].filter(ref=>get(ref).layer==='Bottom').some(ref=>done.has(ref)))throw Error('default mark crossed sides');toggleSelectionMark();if([...selected].some(ref=>done.has(ref)))throw Error('current side not unmarked');
$('bothsides').checked=true;toggleSelectionMark();if(![...selected].every(ref=>done.has(ref)))throw Error('optional both-side mark failed');toggleSelectionMark();if([...selected].some(ref=>done.has(ref)))throw Error('optional both-side unmark failed');$('bothsides').checked=false;
