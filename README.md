...
- `examples/placement.csv`、`bom.csv`、`bom.xlsx`：四元件演示数据。
...
- 左侧物料清单按**当前层及筛选条件**列出，点击后高亮，保持视图；完成按钮作用于正反面全部已选元件。
...
“标记已贴”和“取消标记”包含正反面全部已选元件，包括红色候选。红色提示代表需要人工核对，按钮旁显示候选数量。单击即完成模式仍只标记实际点击的单个元件。
